			...
			<div style="margin: 0 auto; margin-top: 10px; width: 950px;">
				...
				<table class="table table-bordered">
					<c:forEach items="${page.list}" var="o">
						<tbody>
							<tr class="success">
								<th colspan="5">订单编号:${o.oid} 总金额:¥${o.total}元 <c:if test="${o.states==1}">
										<a href="${pageContext.request.contextPath}/orderAction_findOrderByOid?oid=${o.oid}">付款</a>
									</c:if> <c:if test="${o.states==2}">未发货</c:if> <c:if test="${o.states==3}">
										<a href="">签收</a>
									</c:if> <c:if test="${o.states==4}">结束</c:if>
								</th>
							</tr>
							<tr class="warning">
								<th>图片</th>
